...
router.get('/get', auth.authenticateToken, (req, res, next) => {
    var query = `SELECT p.id, p.name, p.description, p.price, p.status, c.id AS categoryId, c.name AS categoryName FROM product AS p 
...
router.get('/getById/:id', auth.authenticateToken, (req, res, next) => {
    const id = req.params.id;
    var query = "SELECT id, name FROM product WHERE id=? AND status='true'";
    connection.query(query, [id], (err, results) => {
        if (!err) {
            if (results.length <= 0) {
